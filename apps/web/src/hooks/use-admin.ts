...
export function useAdminQrCodes(query: { page: number; pageSize: number; search?: string }) {
  return useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResult<QrCode & { user: { id: string; email: string; name: string } }>>(
        '/admin/qr-codes',
        query,
      ),
    placeholderData: (prev) => prev,
...
export function useDeleteAdminQrCode() {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ['admin', 'qr-codes'], exact: false }),
  });
}
...
export function useAuditLogs(query: {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResult<AuditLogEntry>>('/admin/audit-logs', query as QueryParams),
    placeholderData: (prev) => prev,
  });
}
...
export function useDeleteBackup() {
  ...
  return useMutation({
    mutationFn: (fileName: string) =>
      api.delete<{ success: boolean }>(`/admin/backups/${fileName}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'backups'] }),
  });
}
